...
def main():
    f = open("data/running_time_parallel_openmp_and_mpi_2_omp_threads.txt", "r")
    lines = f.readlines()
    f.close()
    x_axis = list()
    y_axis_time = list()
    ...
    ax.set_title(f'Running time parallel algorithm using OpenMP and MPI with 2 OMP threads')
    ax.set_xlabel(f'N')
